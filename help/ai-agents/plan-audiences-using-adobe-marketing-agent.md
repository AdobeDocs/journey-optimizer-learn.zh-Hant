---
title: 使用 Adobe Marketing Agent 進行客群規劃
description: 瞭解如何在 Teams 中使用 Adobe Marketing Agent for Microsoft 365 Copilot 輕鬆規劃客群，從而實現有效的行銷活動準備。
feature: Audiences
role: User
level: Beginner
doc-type: Feature Video
duration: 194
last-substantial-update: '2026-05-27T00:00:00.000Z'
jira: KT-21431
autotag-review: '2026-05-27T22:31:08.526Z'
TQID: 'https://experienceleague.adobe.com/NIUK1r-sCX7eFGG4-0OWy-JxMPrFh57dOMBB9rTMY-A'
product_v2:
  - id: cb954087-f4fc-4456-afb9-e939cabcdc79
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2:
  - id: bbbea26f-9621-49eb-9ab8-e06fb3bbce8c
  - id: e1e0219c-f879-479f-8427-888ed2a6e9c2
source-git-commit: debae556440c9bd902b0adb7590db960f42b3bf2
workflow-type: ht
source-wordcount: 190
ht-degree: 100%

---


# 使用 Adobe Marketing Agent for Microsoft 365 Copilot 規劃客群

瞭解 Adobe Marketing Agent for Microsoft 365 Copilot 如何直接在 Microsoft Teams 中簡化客群規劃。此工具可讓行銷人員識別忠誠度客群、檢查歷程譜系及驗證行銷活動候選者，而無需切換平台或向營運團隊提出請求。

此影片示範選取、分析和準備行銷活動客群的流程。它會展示代理程式如何產生可操作的深入解析、顯示相關建議，並提供客群與歷程之間的譜系視覺效果。行銷人員可以有效地管理資料並簡化工作流程，確保目標定位更聚焦且行銷活動結果更佳。

觀看以下內嵌影片以瞭解 Adobe Marketing Agent 的現場示範。

>[!VIDEO](https://video.tv.adobe.com/v/3491194/?captions=chi_hant&learn=on&enablevpops)

如需有關如何開始使用的詳細資訊，請觀看[開始使用 Adobe Marketing Agent for Microsoft 365 Copilot](https://experienceleague.adobe.com/zh-hant/docs/platform-learn/tutorials/ai-assistant/adobe-marketing-agent/adobe-marketing-agent-get-started){target="_blank"}。
若要瞭解行銷人員如何使用 Adobe Marketing Agent for Microsoft 365 Copilot 來規劃行銷活動、檢閱績效及監視飛行中歷程，請觀看[使用 Adobe Marketing Agent for Microsoft 365 Copilot 的行銷工作流程](https://experienceleague.adobe.com/zh-hant/docs/platform-learn/tutorials/ai-assistant/adobe-marketing-agent/adobe-marketing-agent-workflows){target="_blank"}。
